...
public class Bug_Bullet : MonoBehaviour
{
    ...
    public void Shoot()
    {
        GameObject B = Instantiate(Bullet, Pos.transform.position, Quaternion.Euler(Pos.transform.rotation.x, Pos.transform.rotation.y, Pos.transform.rotation.z + ShootPath));
        B.GetComponent<Rigidbody2D>().AddRelativeForce(new Vector2(ShootPower, 0));
    }
